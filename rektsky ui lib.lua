...
if isfolder("Lime") == false then
    makefolder("Lime")
end

if isfolder("Lime/sound") == false then
    makefolder("Lime/sound")
end

if isfolder("Lime/sound/mc") == false then
    makefolder("Lime/sound/mc")
end

if isfolder("Lime/assets") == false then
    makefolder("Lime/assets")
end

if isfolder("Lime/config") == false then
    makefolder("Lime/config")
end

if isfolder("Lime/scripts") == false then
    makefolder("Lime/scripts")
end

local sliderapi = {}

local foldername = "Lime/config"
local conf = {
...
local function getcustomassetfunc(path)
    if not betterisfile(path) then
        ...
        local req = requestfunc({
            Url = "https://raw.githubusercontent.com/8pmX8/rektsky4roblox/main/"..path:gsub("Lime/assets", "assets"),
            Method = "GET"
        })
...
local function getcustomassetfuncforsounds(path)
    if not betterisfile(path) then
        ...
        local req = requestfunc({
            Url = "https://raw.githubusercontent.com/8pmX8/rektsky4roblox/main/"..path:gsub("Lime/sound", "sound"),
            Method = "GET"
        })
        writefile(path, req.Body)
    end
...
local function playdingsound(istrue) 
    if lib["Sounds"] then
        if istrue then
            playsound(getcustomassetfuncforsounds("Lime/sound/enable.mp3") or getasset("Lime/sound/enable.mp3"))
        else
            playsound(getcustomassetfuncforsounds("Lime/sound/disable.mp3") or getasset("Lime/sound/disable.mp3"))
        end
    end
end
...
function lib:CreateWindow()
    ...
    function lib:CreateTab(title, color, asset)
        ...
        assetthing.Image = getcustomassetfunc("Lime/assets/" .. asset .. ".png") or getasset("Lime/assets/" .. asset .. ".png")
        local tabtable = {
            ["Toggles"]={}
        }
